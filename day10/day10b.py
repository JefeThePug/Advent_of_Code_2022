...
arr = [1] + [int(i) if i.lstrip("-").isdigit() else 0 for cmd in cmds for i in cmd.split()]
total = [sum(arr[:i+1]) for i in range(len(arr))]

output = []
for i in range(240):
    if abs(i%40 - total[i]) > 1:
        output.append(".")
    else:
        output.append("#")

...
def fully_contained(elf1: str, elf2: str) -> bool:
    s1, e1 = map(int,elf1.split("-"))
    s2, e2 = map(int,elf2.split("-"))
    first_in_second = set(range(s1, e1+1)).issubset(range(s2, e2+1))
    second_in_first = set(range(s2, e2+1)).issubset(range(s1, e1+1))
    return first_in_second or second_in_first

def sum_contained(arr: list) -> int:
    return sum([fully_contained(*group.split(",")) for group in arr])

with open("input.txt") as f:
    print(sum_contained(f.read().split("\n")[:-1]))

# Total pairs with complete subsets: 534

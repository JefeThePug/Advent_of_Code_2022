...
def up(grid: list, n:tuple) -> int:
    col, row = n
    return max(x[row] for x in grid[:col])
def down(grid: list, n:tuple) -> int:
    col, row = n
    return max(x[row] for x in grid[col+1:])
def left(grid: list, n:tuple) -> int:
    col, row = n
    return max(grid[col][:row])
def right(grid: list, n:tuple) -> int:
    col, row = n
    return max(grid[col][row+1:])
...
for col in range(1, len(grid) - 1):
    for row in range(1, len(grid[0]) - 1):
        if any(f(grid,(col,row)) < grid[col][row] for f in directions):
            visible += 1
